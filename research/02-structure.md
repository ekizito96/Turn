...
**For Turn:** Turn adopts an object-oriented model: the **agent** is the central object; it has a **context** object (bounded buffer) and a **memory** object (key-value store), and it **calls tools** (message-like suspension/resume). So tools and memory are the agent's objects; we don't go full Smalltalk, but the metaphor fits.
