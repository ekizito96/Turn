...
use crate::{offset_to_line_col, line_col_to_offset};
use std::sync::RwLock;
    ...
    fn find_identifier(&self, source: &str, offset: usize) -> Option<(String, Span)> {
        ...
        loop {
            match lexer.next_token() {
                Ok(t) => {
                    if let Token::Eof = t.token { break; }
...
impl LanguageServer for Backend {
    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        self.documents.write().unwrap().insert(params.text_document.uri.clone(), params.text_document.text.clone());
    ...
    async fn did_change(&self, params: DidChangeTextDocumentParams) {
        ...
        if let Some(change) = params.content_changes.first() {
            self.documents.write().unwrap().insert(params.text_document.uri.clone(), change.text.clone());
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let mut items: Vec<CompletionItem> = keywords.into_iter().map(|k| {
            CompletionItem {
                label: k.to_string(),
        ...
        if let Some(source) = source_opt {
             if let Some(offset) = line_col_to_offset(&source, (position.line + 1) as usize, (position.character + 1) as usize) {
    ...
    async fn goto_definition(&self, params: GotoDefinitionParams) -> Result<Option<GotoDefinitionResponse>> {
        ...
        let offset = match line_col_to_offset(&source, (position.line + 1) as usize, (position.character + 1) as usize) {
            Some(o) => o,
            None => return Ok(None),
        };
            ...
            if let Some(def_span) = analysis.usages.get(&span.start) {
                 ...
                 return Ok(Some(GotoDefinitionResponse::Scalar(Location {
                     ...
                     range: Range {
                         ...
                         end: Position { line: (line - 1) as u32, character: (col - 1) as u32 },
                     }
                 })));
            }
        }
